...
test(
  ...
  async () => {
    ...
    process.stderr.write(
      `[anthropic-smoke] ok — tokens=${response.usage.inputTokens}/${response.usage.outputTokens} cost=$${response.usage.costUsd.toFixed(6)} categories=${populatedFields.join(',')}\n`,
    );
  },
);
