...
// Live OpenAI smoke test — real API call. Skipped if OPENAI_API_KEY is
// not set. Mirror of test/anthropic.smoke.js for the OpenAI adapter.
// Run via `npm run smoke`.

const test = require('node:test');
const assert = require('node:assert/strict');
const { OpenAiAdapter } = require('../src/model/openai.js');
const { EMIT_CMB_TOOL } = require('../src/core/loop.js');

const hasKey = Boolean(process.env.OPENAI_API_KEY);

test(
  'live smoke: OpenAI adapter issues real API call and surfaces emit_cmb tool_use',
